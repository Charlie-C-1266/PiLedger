...
- The dashboard is a React SPA, so user-supplied strings (account names, notes, merchants) are rendered as JSX text and escaped automatically by React, preventing stored XSS. The app does not use `dangerouslySetInnerHTML` for user data.
