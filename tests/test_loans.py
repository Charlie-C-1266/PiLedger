...
def test_create_loan_account(alice):
    resp = alice.post("/api/accounts", json={
        "name": "Mortgage", "type": "loan", "interest_rate": 5.0, "color": "#ef4444",
    })
...
def test_loan_appears_in_account_list(alice):
    alice.post("/api/accounts", json={"name": "Car loan", "type": "loan", "interest_rate": 6.5})
...
def test_loan_subtracts_from_net_worth(alice):
    cur = alice.post("/api/accounts", json={"name": "Current", "type": "current"}).json()["id"]
    sav = alice.post("/api/accounts", json={"name": "Saver",   "type": "savings"}).json()["id"]
    mtg = alice.post("/api/accounts", json={"name": "Mortgage", "type": "loan"}).json()["id"]
...
def test_summary_loans_only_negative_net_worth(alice):
    aid = alice.post("/api/accounts", json={"name": "Credit card", "type": "loan"}).json()["id"]
...
def test_loan_user_isolation(alice, bob):
    aid = alice.post("/api/accounts", json={"name": "Alice's loan", "type": "loan"}).json()["id"]
...
def test_loan_interest_accrues_with_no_payments(alice):
    ...
    aid = alice.post("/api/accounts", json={"name": "Mortgage", "type": "loan", "interest_rate": 6.0}).json()["id"]
...
def test_loan_payment_reduces_balance(alice):
    ...
    aid = alice.post("/api/accounts", json={"name": "Mortgage", "type": "loan", "interest_rate": 5.0}).json()["id"]
    ...
    alice.post("/api/budget", json={
        "account_id": aid, "name": "Monthly payment",
        "amount": -1_200.0, "frequency": "monthly",
    })
...
def test_projection_includes_net_worth_array(alice):
    cur = alice.post("/api/accounts", json={"name": "Current", "type": "current"}).json()["id"]
    mtg = alice.post("/api/accounts", json={"name": "Mortgage", "type": "loan", "interest_rate": 5.0}).json()["id"]
...
def test_loan_budget_item_round_trip(alice):
    ...
    aid = alice.post("/api/accounts", json={"name": "Car loan", "type": "loan", "interest_rate": 6.0}).json()["id"]
    item = alice.post("/api/budget", json={
        "account_id": aid, "name": "Repayment",
        "amount": -350.0, "frequency": "monthly",
    }).json()
