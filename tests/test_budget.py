...
def test_create_budget_requires_auth(client):
    assert client.post("/api/budget", json={
        "account_id": 1, "name": "X", "amount": 100.0, "frequency": "monthly",
    }).status_code == 401
...
def test_create_budget_returns_201_and_appears_in_list(alice):
    aid = alice.post("/api/accounts", json={"name": "Current", "type": "current"}).json()["id"]
    r = alice.post("/api/budget", json={
        "account_id": aid, "name": "Salary",
        "amount": 3000.0, "frequency": "monthly",
    })
...
def test_user_cannot_post_budget_against_another_users_account(alice, bob):
    ...
    r = bob.post("/api/budget", json={
        "account_id": alice_acc, "name": "Sneaky",
        "amount": 100.0, "frequency": "monthly",
    })
    assert r.status_code == 404
...
def test_list_budget_user_isolation(alice, bob):
    aid = alice.post("/api/accounts", json={"name": "A", "type": "current"}).json()["id"]
...
def test_user_cannot_update_another_users_budget(alice, bob):
    aid = alice.post("/api/accounts", json={"name": "A", "type": "current"}).json()["id"]
    bid = alice.post("/api/budget", json={
        "account_id": aid, "name": "Salary",
        "amount": 1000.0, "frequency": "monthly",
    }).json()["id"]
...
def test_deleting_account_cascades_to_its_budget_items(alice):
    ...
    keep_acc = alice.post("/api/accounts", json={"name": "Keep", "type": "current"}).json()["id"]
    drop_acc = alice.post("/api/accounts", json={"name": "Drop", "type": "current"}).json()["id"]

    keep_item = alice.post("/api/budget", json={
        "account_id": keep_acc, "name": "Keep me",
        "amount": 100.0, "frequency": "monthly",
    }).json()["id"]
    alice.post("/api/budget", json={
        "account_id": drop_acc, "name": "Drop me",
...
def test_create_budget_rejects_unknown_frequency(alice):
    aid = alice.post("/api/accounts", json={"name": "A", "type": "current"}).json()["id"]
    r = alice.post("/api/budget", json={
        "account_id": aid, "name": "X",
        "amount": 100.0, "frequency": "daily",
    })
    assert r.status_code == 400
...
def test_create_budget_rejects_amount_above_max_money(alice):
    ...
    r = alice.post("/api/budget", json={
        "account_id": aid, "name": "X",
        "amount": MAX_MONEY + 1, "frequency": "monthly",
    })
    assert r.status_code == 400
...
def test_negative_amount_accepted_for_loan_payments(alice):
    ...
    aid = alice.post("/api/accounts", json={"name": "A", "type": "current"}).json()["id"]
    r = alice.post("/api/budget", json={
        "account_id": aid, "name": "Loan payment",
...
def _seed_item(alice):
    aid = alice.post("/api/accounts", json={"name": "A", "type": "current"}).json()["id"]
    return alice.post("/api/budget", json={
        "account_id": aid, "name": "Original",
        "amount": 1000.0, "frequency": "monthly",
    }).json()
