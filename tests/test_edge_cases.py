...
def test_very_large_balance_is_stored_accurately(alice):
    aid = alice.post("/api/accounts", json={"name": "BigBucks", "type": "current"}).json()["id"]
...
def test_summary_totals_five_accounts_accurately(alice):
    for i in range(5):
        aid = alice.post("/api/accounts", json={"name": f"A{i}", "type": "current"}).json()["id"]
...
def test_account_name_with_unicode(alice):
    ...
    resp = alice.post("/api/accounts", json={"name": name, "type": "savings", "interest_rate": 2.0})
    assert resp.status_code == 201
    assert alice.get("/api/accounts").json()[0]["name"] == name
...
def test_interest_rate_fractional_precision(alice):
    aid = alice.post("/api/accounts", json={"name": "Saver", "type": "savings", "interest_rate": 4.75}).json()["id"]
    alice.post(f"/api/accounts/{aid}/balance", json={"balance": 1000.0})
...
def test_high_interest_rate_does_not_crash(alice):
    aid = alice.post("/api/accounts", json={"name": "HighRate", "type": "savings", "interest_rate": 99.9}).json()["id"]
    alice.post(f"/api/accounts/{aid}/balance", json={"balance": 1000.0})
...
def test_projection_milestone_values_are_consistent(alice):
    ...
    aid = alice.post("/api/accounts", json={"name": "S", "type": "savings", "interest_rate": 4.0}).json()["id"]
...
def test_delete_then_create_new_account_has_no_leftover_balance(alice):
    ...
    new_aid = alice.post("/api/accounts", json={"name": "New", "type": "current"}).json()["id"]
...
def test_balance_sequence_only_latest_counts_in_summary(alice):
    aid = alice.post("/api/accounts", json={"name": "X", "type": "savings"}).json()["id"]
...
def test_update_account_does_not_affect_balance_history(alice):
    aid = alice.post("/api/accounts", json={"name": "Old Name", "type": "current"}).json()["id"]
...
def test_exactly_8_char_password_is_accepted(client):
    resp = client.post("/api/auth/register", json={"username": "alice", "password": "12345678"})
    assert resp.status_code == 201


def test_7_char_password_is_rejected(client):
    resp = client.post("/api/auth/register", json={"username": "alice", "password": "1234567"})
...
def test_projection_point_count_for_all_supported_periods(alice):
    ...
    for months in (12, 24, 60):
        ...
        assert len(data[0]["points"]) == months + 1, f"Expected {months + 1} points for months={months}"
...
def test_history_all_includes_all_accounts_with_entries(alice):
    for name, balance in (("A1", 100.0), ("A2", 200.0), ("A3", 300.0)):
        aid = alice.post("/api/accounts", json={"name": name, "type": "current"}).json()["id"]
...
def test_multiple_savings_accounts_each_appear_in_projections(alice):
    for name, rate in (("Saver A", 3.0), ("Saver B", 4.5)):
        aid = alice.post("/api/accounts", json={"name": name, "type": "savings", "interest_rate": rate}).json()["id"]
        alice.post(f"/api/accounts/{aid}/balance", json={"balance": 1000.0})
