...
- **Budget screen polish — accessibility, end-to-end coverage, and docs.** Final phase of the zero-based envelope Budget screen. Accessibility sweep over the interactive controls: the envelope and group "Edit" buttons gained `aria-label`s naming what they edit (the envelope-label button only carried a `title`, and the group "Edit" button was unlabelled for screen readers), and the loading placeholder is now a `role="status"` live region (the period toggle was already a labelled `radiogroup`, and the steppers and sliders already carried `aria-label`s). Adds the first Playwright coverage for the screen (`tests/e2e/test_budget.py`): one test builds a budget through the real add-income/group/envelope flow and asserts the envelope's live `spent` reflects a seeded current-month transaction; another seeds a budget via the API, then drives the UI to check the period toggle rescales every figure (monthly → yearly ×12) and that dragging an envelope slider updates the budgeted figure, group total and hero allocation live. Docs updated to retire the old "Budget Planner" and describe the new screen: `docs/frontend.md` (sixth SPA route, `useBudget`/`useBudgetEdit`, the budget charts and modals, Recharts-vs-hand-rolled note) and `docs/architecture.md` (the R12 "Budgeting" requirement row, and R13 no longer mentions loan payments as budget items). Affected files: `frontend/src/components/budget/EnvelopeRow.tsx`, `frontend/src/components/budget/GroupCard.tsx`, `frontend/src/screens/Budget.tsx` (aria-labels + loading `role="status"`), `tests/e2e/test_budget.py` (new), `docs/frontend.md`, `docs/architecture.md`.

- **Budget-vs-actual trend chart (frontend).** Adds the final piece of the Budget screen: a `BudgetTrend` card at the foot of the left column, plotting the last 6 months of budgeted vs actual spend from the `GET /api/budget` `history`. It's a lightweight CSS-grid bar chart (plain divs, no chart lib) — one column per month with a paired "Budgeted" bar (the current flat allocation) and "Spent" bar, the spent bar turning red (`--pl-down`) when that month overspent. Bar heights are ratios against the 6-month max, so the period toggle leaves them unchanged and only rescales the hover tooltips; the sub-heading reads "normalised to <period>". The card hides itself when there's no history yet (no envelopes). Affected files: `frontend/src/components/budget/BudgetTrend.tsx` + `BudgetTrend.module.css` (new), `frontend/src/screens/Budget.tsx` (trend wired into the left column).

- **Budget right rail — safe-to-spend, allocation donut, spent-so-far (frontend).** The Budget screen becomes its designed two-column layout (`1fr 360px`, sticky rail on desktop, single column below 900px) and the right column lands. The collapse point is 900px rather than the handoff's 720px because the app's persistent side nav eats width the design assumed was free — at ~720px the two-column left card was only ~210px, cramping the hero (its period toggle and the income steppers spilled outside their cards). The left column is also a CSS container (`container-type: inline-size`) so the hero, period toggle and income card respond to the column's real width, not the viewport. `SafeToSpendCard` is an accent-gradient card showing what's left in **flexible** groups only (Σ `budgeted − spent`, clamped ≥ 0) in the active period, with a "£X/day for N days" pace caption in the monthly view. `AllocationDonut` ("Where it goes") renders one slice per group through the shared `charts/Donut`, with a centre that shows the allocated total (or the hovered group's total/percent) and a cross-highlighting legend (hovering a row or slice dims the rest); it falls back to a hint when nothing is allocated yet. `SpentSoFarCard` shows total spend across all envelopes with an "X% of plan" `HBar` and an "of £X across N envelopes" caption. Every figure recomputes live from the same `["budget"]` cache the envelope sliders patch, so dragging a slider updates the rail too. Affected files: `frontend/src/components/budget/SafeToSpendCard.tsx` + `SafeToSpendCard.module.css` (new), `frontend/src/components/budget/AllocationDonut.tsx` + `AllocationDonut.module.css` (new), `frontend/src/components/budget/SpentSoFarCard.tsx` + `SpentSoFarCard.module.css` (new), `frontend/src/screens/Budget.tsx` (two-column layout + rail wiring + derived totalSpent/flexRemaining/envelopeCount), `frontend/src/screens/Budget.module.css` (grid layout).

- **Budget envelope group cards — live sliders + group/envelope CRUD (frontend).** Replaces the placeholder group list with the designed `GroupCard`: a colour swatch, name, Fixed/Flexible tag and the group's budgeted total, over its `EnvelopeRow`s. Each envelope row shows a "£X spent / £Y budgeted" line (the spent figure turns red when overspent), a spend `HBar` (group-coloured, red when over), and a range slider (step 5, accent = group colour). Dragging a slider optimistically patches the `["budget"]` cache via the Phase-8 `useBudgetEdit` hook — so the group total, allocated, hero "Left to budget" and balance bar all move live — and persists `PUT /api/budget/envelopes/{id}` debounced ~400ms. Full CRUD via modals built on the shared `AddModal` styles: an `AddGroupModal` (name, preset+hex colour picker reusing `theme/swatches`, Fixed/Flexible toggle, delete) and an `AddEnvelopeModal` (name, category picker that hides categories already enveloped, group selector, monthly budget, delete) — opened from each card's "Edit" / "Add envelope" controls and a bottom "Add group" button (also the empty-state action). Affected files: `frontend/src/components/budget/GroupCard.tsx` + `GroupCard.module.css` (new), `frontend/src/components/budget/EnvelopeRow.tsx` + `EnvelopeRow.module.css` (new), `frontend/src/components/budget/AddGroupModal.tsx` (new), `frontend/src/components/budget/AddEnvelopeModal.tsx` (new), `frontend/src/screens/Budget.tsx` (group cards + modal wiring; placeholder group rendering removed), `frontend/src/screens/Budget.module.css` (dead placeholder styles dropped).

- **Budget income card with live ±£50 steppers (frontend).** Replaces the placeholder income list with the designed `IncomeCard`: a header (title + "what you have to assign each month/week/year" sub, period-aware) and a right-aligned green total, then one row per manual income line (label, period-scaled amount, and a `Stepper`), plus an accent "Add income" pill. Each stepper nudges the line's monthly amount by ±£50 (clamped at 0) and edits are **live and persisted**: the change optimistically patches the cached `["budget"]` payload so the income total, hero "Left to budget", balance bar and stat row all recompute instantly, while the matching `PUT /api/budget/income/{id}` is debounced ~400ms (mirroring the Goals slider) and reconciles on success. Income remains manual — never derived from transactions. New reusable pieces: a `useBudgetEdit` hook (`patch` for the synchronous cache rewrite + keyed-debounced `persist`) that the envelope sliders will reuse, a `Stepper` component, and a `MinusIcon`. Affected files: `frontend/src/components/budget/IncomeCard.tsx` + `IncomeCard.module.css` (new), `frontend/src/components/budget/Stepper.tsx` + `Stepper.module.css` (new), `frontend/src/hooks/useBudget.ts` (`useBudgetEdit`), `frontend/src/screens/Budget.tsx` (income card wired in), `frontend/src/components/icons/index.tsx` (`MinusIcon`).
...
- **Documentation refreshed to match the current React/Vite design.** Several docs still described the pre-rebuild stack and the retired "Budget Planner". `docs/architecture.md`'s frontend paragraph and file-structure tree were rewritten from the old vanilla-JS / Chart.js / Inter / `app.js`+`style.css` layout to the real one — a React 19 + TypeScript SPA built by Vite into `src/static/dist/`, Recharts plus hand-rolled SVG/CSS charts, self-hosted Plus Jakarta Sans + JetBrains Mono, the six screens, the `frontend/` source tree, and the standalone `login`/`guide` static pages. `docs/getting-started.md` and `docs/deployment.md` gained the missing frontend build step (`cd frontend && npm ci && npm run build`) and a Node 20+ prerequisite, since the SPA bundle is gitignored and a from-source run otherwise serves no dashboard. `docs/api-reference.md` dropped the stale "CRUD added in later phases" note from the Budget section, documented the public `GET /api/docs/{slug}` guide endpoint, and updated the SPA-routing list to six routes (adding `/budget` and noting the standalone `/login` + `/guide` pages). `docs/testing.md`'s static-asset smoke tests no longer curl the deleted `style.css`/`app.js`. Affected files: `docs/architecture.md`, `docs/getting-started.md`, `docs/deployment.md`, `docs/api-reference.md`, `docs/testing.md`, `docs/frontend.md`.

- **Budget envelope sliders no longer run away into the billions when dragged up.** Each envelope slider's maximum was derived from the envelope's own current budget (`max ≈ 2 × budgeted`), and the slider's value *is* that budget — so dragging the thumb to the right doubled the amount, which on the next render doubled the maximum, letting the next drag double it again. A few pulls to the right ran the figure up into the billions (dragging left behaved, since both value and max shrank together). The slider top is now a stable ceiling independent of the value being dragged: a single envelope tops out at the household's monthly income (the zero-based ceiling), with a £1,000 floor so the control still works before any income is entered, and the current budget folded in only so an over-income amount set in the edit modal stays representable — rounded up to a tidy £100. Affected files: `frontend/src/components/budget/EnvelopeRow.tsx` (stable `sliderMax` from a new `incomeTotal` prop), `frontend/src/components/budget/GroupCard.tsx` + `frontend/src/screens/Budget.tsx` (thread the monthly income total down to the sliders).
